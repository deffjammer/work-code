...
/* Is string 'suffix' a suffix of string 's'? */
static int strsuffix(const char *s, const char *suffix)
{
	return streq(s + strlen(s) - strlen(suffix), suffix);
}
...
static int parse_distmap_line(unsigned int node, char *buf)
{
	...
	for (c = bitmask_first(cpus); c < ncpus; c = bitmask_next(cpus, c + 1))
		for (n = 0; n < nmems; n++)
...
/* [optional] Return Memory Node closest to cpu within a cpuset*/
int cpuset_cpu2node_c_rel(int cpu)
{
       	struct cpuset *cp_tofree = NULL;
       	const struct cpuset *cp1 = resolve_cp(NULL, &cp_tofree);
       	int sys_node, c_rel_node= -1;
	...
	if (!cp1)
		goto err;
	sys_node = cpuset_cpu2node(cpu);
	c_rel_node = cpuset_c_sys_to_rel_mem(NULL,sys_node);
err:
       cpuset_free(cp_tofree);
       return c_rel_node;
}
...
static int cpuset_would_crash_kernel(const struct cpuset *cp)
{
	...
	if (susceptible_system) {
		...
		for (cpu = 0; cpu < ncpus; cpu += siblings) {
			int s, num_set = 0;
...
static int import_list(const char *tok, const char *arg, struct bitmask *bmp,
						char *emsg, int elen)
{
